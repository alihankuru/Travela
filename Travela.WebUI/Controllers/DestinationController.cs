...
namespace Travela.WebUI.Controllers
{
    public class DestinationController : Controller
    {
        ...
        public async Task<IActionResult> DestinationList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7108/api/Destination");
        ...
        public async Task<IActionResult> CreateDestination(CreateDestinationDto createDestinationDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7108/api/Destination", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7108/api/Destination?id=" + id);
            return RedirectToAction("DestinationList");
        }
        ...
        public async Task<IActionResult> UpdateDestination(int id) {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7108/api/Destination/GetDestination?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateDestination(UpdateDestinationDto updateDestinationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData,Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7108/api/Destination", stringContent);
            return RedirectToAction("DestinationList");
        }
